...
import { readStudioDrafts, removeStudioDraft, getStudioDraftsChangeEvent, type StudioDraft } from "@/lib/studioDrafts";
...
const IncompleteCreations = () => {
  const [creations, setCreations] = useState<StudioDraft[]>([]);
  ...
  useEffect(() => {
    const loadDrafts = () => {
      const drafts = readStudioDrafts().sort((a, b) => +new Date(b.updated_at) - +new Date(a.updated_at));
      setCreations(drafts);
      setLoading(false);
    };
    ...
    window.addEventListener(changeEvent, loadDrafts);
    window.addEventListener("storage", loadDrafts);
    return () => {
      window.removeEventListener(changeEvent, loadDrafts);
  ...
  const handleDelete = async () => {
    ...
    removeStudioDraft(deleteTarget.id);
    setCreations((prev) => prev.filter((c) => c.id !== deleteTarget.id));
    toast.success("Eliminato");
    setDeleteTarget(null);
  };

  const handleResume = (item: StudioDraft) => {
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    <p className="text-[11px] text-muted-foreground">{formatDate(item.updated_at)}</p>
