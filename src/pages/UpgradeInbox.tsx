...
const UpgradeInbox = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`upgrade-inbox-msg-${selectedConv.id}`)
      .on("postgres_changes", {
        event: "INSERT",
        schema: "public",
        table: "upgrade_messages",
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("upgrade-inbox-convos")
      .on("postgres_changes", { event: "*", schema: "public", table: "upgrade_conversations" }, () => loadConversations())
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "upgrade_messages" }, () => loadConversations())
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [authorized]);
  ...
  const handleSend = async () => {
    ...
    setInput("");
    setSending(true);
    const { error } = await supabase.from("upgrade_messages").insert({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await supabase.from("upgrade_messages").insert({
      conversation_id: selectedConv.id,
      sender_id: user.id,
  ...
  const markComplete = async () => {
    ...
    toast({ title: "Chiusa", description: "Conversazione chiusa." });
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-card">
            ...
            <div className="flex items-center gap-2">
              {selectedConv.status === "completed" && (
                <Badge className="bg-green-500/10 text-green-600 border-green-500/30">Chiusa</Badge>
              )}
            ...
            {messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex ${isFounder ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${isFounder ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"}`}>
                    {msg.image_url && <img src={msg.image_url} alt="Allegato" className="rounded-lg mb-2 max-w-full max-h-48 object-cover" />}
                    {msg.content && <p className="text-sm whitespace-pre-wrap">{msg.content}</p>}
                    <p className={`text-[10px] mt-1 ${isFounder ? "text-primary-foreground/60" : "text-muted-foreground"}`}>
          ...
          {selectedConv.status === "open" && (
            <div className="p-3 border-t border-border flex items-center gap-2 bg-card">
              ...
              <Input
                ...
                className="flex-1"
              />
              <Button size="icon" className="shrink-0 bg-primary hover:bg-primary/90" onClick={handleSend} disabled={!input.trim() || sending}>
    ...
    <div className="flex flex-col flex-1 overflow-hidden bg-background">
      ...
      <main className="flex-1 overflow-y-auto px-4 pt-4 pb-20">
        <div className="max-w-2xl mx-auto">
          <PageTitle title="Upgrade — Inbox" backTo="/admin" />

          {conversations.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground text-sm">
              Nessuna proposta di upgrade ancora.
            </div>
          ) : (
            <div className="space-y-2">
              {conversations.map((conv) => {
                ...
                return (
                  <Card key={conv.id} className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => setSelectedConv(conv)}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {conv.status === "completed" && (
                            <Badge variant="outline" className="text-[10px] ml-2 bg-green-500/10 text-green-600 border-green-500/30 shrink-0">✓</Badge>
                          )}
                        </div>
                      </div>
                    </CardContent>
